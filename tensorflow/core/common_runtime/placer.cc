...
class ColocationGraph {
 public:
  ...
  // 재귀함수를 써서 루트 노드를 찾아간다.
  // exiting condition은 자신과 자신의 패런트 노드의 id가 같으면 return 이다.
  // 루트노드만 위의 조건을 만족하는 듯하다.
...
{
  printf("Placer ctor entry\n");
  fflush(stdout);
  for (int i = 0; i < num_virtual_gpu; ++i) {
    gpu_device_ids.push_back(-1);
    printf("Placer, Initialize gpu_device_ids value to -1\n");
  }
  if (options != nullptr) {
    gangmuk_job_id = options_->config.gangmuk_job_id();
    CHECK_GT(gangmuk_job_id, -1);
    CHECK_LT(gangmuk_job_id, 4);
    printf("Placer ctor, gangmuk_job_id: %d\n", gangmuk_job_id);
    fflush(stdout);
  }
  else {
    printf("Placer ctor, option == nullptr\n");
    fflush(stdout);
  }
}
...
Status Placer::Run() {
  printf("Placer,Run(),gangmuk_job_id: %d\n", gangmuk_job_id);
  ...
  for (Node* node : graph_->op_nodes()) {
    ...
    for (int i = 0; i < (*devices).size(); i++) {
      existing_devices.insert((*devices)[i]->name());
    }
    if (!status.ok()) {
      return AttachDef(
    ...
    std::string assigned_device_name = "none";
    ...
    if (IsMetadata(node)) {
      ...
      if (CanAssignToDevice(input->assigned_device_name(), *devices)) {
        ...
        assigned_device_name = input->assigned_device_name();
      }
    }

    // Can't Assign To input assigned device. Therefore, Provide the default, if necessary.
    if (assigned_device == -1) {
      // default로 0번 디바이스를 assign한다.
      // devices는 위에서 GetDeivcesForNode에서 얻은 device list이다.
      assigned_device_name = (*devices)[0]->name();
      assigned_device = graph_->InternDeviceName(assigned_device_name);
    }
  ...
  for (Node* node : second_pass) {
    ...
    // 제너레이터 노드는 인풋이 없고 아웃풋이 하나만 있는 노드인데, 디바이스를 어떻게 할당할까.
    // 인풋이 있는 노드는 인풋 노드의 디바이스를 그대로 계승해서 자신도 동일한 디바이스에 할당을 한다.
    // 인풋이 없는 제너레이터는 자연스럽게 아웃풋 노드의 디바이스를 그대로 따라서 할당한다.
    // 아웃풋 노드가 할당 받은 디바이스가 없다면, 디폴트 디바이스를 할당한다. 기본적으로는 GPU:0 이다.
    if (IsGeneratorNode(node) && !node->out_edges().empty()) { 
      ...
      if (consumers_on_same_device &&
        ...
        assigned_device_name = output->assigned_device_name();
      }
    }
  ...
  // Device Overwriting
  if (num_virtual_gpu) {
    printf("Placer,Overwriting Process starts\n");
    fflush(stdout);
    int i = 0;
    for (std::set<std::string>::iterator it=existing_devices.begin(); it!=existing_devices.end(); ++it) {
      printf("Placer,existing_devices[%d]: %s\n", i++, (*it).c_str());
      int device_index = graph_->InternDeviceName(*it);
      if ((*it).find("CPU") && cpu_id == -1) {
        cpu_id = device_index;
        printf("Placer,device_name: %s, cpu_id: %d\n", (*it).c_str(), cpu_id);
      }
      else if ((*it).find("GPU:0") && gpu_device_ids[0] == -1) {
        gpu_device_ids[0] = device_index;
        printf("Placer,device_name: %s, gpu_device_ids[0]: %d\n", (*it).c_str(),
               device_index);
      } else if ((*it).find("GPU:1") && gpu_device_ids[1] == -1) {
        gpu_device_ids[1] = device_index;
        printf("Placer,device_name: %s, gpu_device_ids[1]: %d\n", (*it).c_str(),
               device_index);
      } else if ((*it).find("GPU:2") && gpu_device_ids[2] == -1) {
        gpu_device_ids[2] = device_index;
        printf("Placer,device_name: %s, gpu_device_ids[2]: %d\n", (*it).c_str(),
               device_index);
      } else if ((*it).find("GPU:3") && gpu_device_ids[3] == -1) {
        gpu_device_ids[3] = device_index;
        printf("Placer,device_name: %s, gpu_device_ids[3]: %d\n", (*it).c_str(),
               device_index);
      }
    }
 
    printf("Placer,Start Device Assignment Overwriting Process\n");
    fflush(stdout);
    Env::Default()->SleepForMicroseconds(1000);
    for (Node* node : graph_->op_nodes()) {
      if (node->assigned_device_name().find("GPU") != std::string::npos) {
        int overwriting_gpu_id = gpu_device_ids[gangmuk_job_id];
        std::string overwriting_gpu_name = "GPU:"+std::to_string(gangmuk_job_id);
        // printf(
        //     "Placer,Overwriting. %s(%d) => %s(%d) (gangmuk_job_id: %d, "
        //     "NodeName: %s, NodeId: %d)\n",
        //     node->assigned_device_name().c_str(),
        //     node->assigned_device_name_index(), overwriting_gpu_name.c_str(),
        //     overwriting_gpu_id, gangmuk_job_id, node->name().c_str(),
        //     node->id());
        // fflush(stdout);
        AssignAndLog(overwriting_gpu_id, node);
      }
      else {
        printf(
            "Placer,No overwriting for CPU assigned node (gangmuk_job_id: %d, "
            "NodeName: %s, NodeId: %d,)\n",
            gangmuk_job_id, node->name().c_str(), node->id());
        fflush(stdout);
      }
    }
    printf("Finish Device Assignment Overwriting Process\n");
  }
  return Status::OK();
}
...
// Gangmuk
// void Placer::AssignAndLog(int assigned_device, string assigned_device_name,
//                           Node* node, int pass) const {
//   if (num_virtual_gpu && assigned_device_name.find("GPU") != std::string::npos &&
//           (assigned_device == 2 || assigned_device == 3) ) {
//     if (gangmuk_job_id == 0) {
//       printf("Call ReAssignAndLog(2, \"GPU:0\", node, pass)\n");
//       ReAssignAndLog(2, "GPU:0", node, pass); // Reassign this node to GPU:0 (GPU:0 => assigned_device: 2)
//     } else if (gangmuk_job_id == 1) {
//       printf("Call ReAssignAndLog(3, \"GPU:1\", node, pass)\n");
//       ReAssignAndLog(3, "GPU:1", node, pass); // Reassign this node to GPU:1 (GPU:0 => assigned_device: 3)
//     }
//   } else if (assigned_device_name.find("GPU") != std::string::npos &&
//              (assigned_device != 2 || assigned_device != 3)) {
//     printf("Device name: %s, BUT assigned_device_index: %d\n",
//            assigned_device_name.c_str(), assigned_device);
...
// Gangmuk
// void Placer::ReAssignAndLog(int assigned_device, std::string assigned_device_name,
//                           Node* node, int pass) const {
//   printf("=> node(%d,%s),ReAssignAndLog,pass,%d,job_id,%d,device(%d,%s)\n",
//          node->id(), node->name().c_str(), pass,
//          gangmuk_job_id, assigned_device,
//          assigned_device_name.c_str());
//   node->set_assigned_device_name_index(assigned_device);
//   LogDeviceAssignment(node);
// }
