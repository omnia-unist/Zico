...
#ifndef TENSORFLOW_CORE_COMMON_RUNTIME_ORIGINAL_BFC_ALLOCATOR_H_
#define TENSORFLOW_CORE_COMMON_RUNTIME_ORIGINAL_BFC_ALLOCATOR_H_
...
class OriginalBFCAllocator : public Allocator {
 public:
  // Takes ownership of sub_allocator.
  OriginalBFCAllocator(SubAllocator* sub_allocator, size_t total_memory,
               bool allow_growth, const string& name);
  ~OriginalBFCAllocator() override;
 ...
 private:
  ...
  // A ChunkHandle is an index into the chunks_ vector in OriginalBFCAllocator
  ...
  struct Chunk {
    ...
    string DebugString(OriginalBFCAllocator* a,
      ...
      strings::StrAppend(
          ...
          " | in_use: ", in_use());
      if (recurse && prev != OriginalBFCAllocator::kInvalidChunkHandle) {
        Chunk* p = a->ChunkFromHandle(prev);
        strings::StrAppend(&dbg, ", prev: ", p->DebugString(a, false));
      }
      if (recurse && next != OriginalBFCAllocator::kInvalidChunkHandle) {
  ...
  struct Bin {
    ...
    struct ChunkComparator {
      explicit ChunkComparator(OriginalBFCAllocator* allocator)
     ...
     private:
      OriginalBFCAllocator* allocator_;  // The parent allocator
    };
    ...
    Bin(OriginalBFCAllocator* allocator, size_t bs)
  ...
  // OriginalBFCAllocator allocates memory into a collection of disjoint
  ...
  TF_DISALLOW_COPY_AND_ASSIGN(OriginalBFCAllocator);
};

}  // namespace tensorflow

#endif  // TENSORFLOW_CORE_COMMON_RUNTIME_ORIGINAL_BFC_ALLOCATOR_H_
