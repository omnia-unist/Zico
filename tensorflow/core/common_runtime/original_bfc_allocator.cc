...
#include "tensorflow/core/common_runtime/original_bfc_allocator.h"
...
OriginalBFCAllocator::OriginalBFCAllocator(SubAllocator* sub_allocator, size_t total_memory,
...
OriginalBFCAllocator::~OriginalBFCAllocator() {
  // Return memory back.
...
OriginalBFCAllocator::Chunk* OriginalBFCAllocator::ChunkFromHandle(ChunkHandle h) {
  DCHECK_GE(h, 0);
  DCHECK_LT(h, static_cast<int>(chunks_.size()));
  return &(chunks_[h]);
}

bool OriginalBFCAllocator::Extend(size_t alignment, size_t rounded_bytes) {
  ...
  OriginalBFCAllocator::Chunk* c = ChunkFromHandle(h);
  c->ptr = mem_addr;
  c->size = bytes;
  c->allocation_id = -1;
...
OriginalBFCAllocator::ChunkHandle OriginalBFCAllocator::AllocateChunk() {
...
void OriginalBFCAllocator::DeallocateChunk(ChunkHandle h) {
  Chunk* c = ChunkFromHandle(h);
  c->next = free_chunks_list_;
  free_chunks_list_ = h;
}

void* OriginalBFCAllocator::AllocateRaw(size_t unused_alignment, size_t num_bytes) {
...
// static
size_t OriginalBFCAllocator::RoundedBytes(size_t bytes) {
...
void* OriginalBFCAllocator::AllocateRawInternal(size_t unused_alignment,
                                        size_t num_bytes,
                                        bool dump_log_on_failure) {

  printf("OriginalBFCAllocator::Allocate_Raw_Internal\n");
...
void* OriginalBFCAllocator::FindChunkPtr(BinNum bin_num, size_t rounded_bytes,
                                 size_t num_bytes) {
  ...
  for (; bin_num < kNumBins; bin_num++) {
    ...
    for (auto citer = b->free_chunks.begin(); citer != b->free_chunks.end();
         ++citer) {
      const OriginalBFCAllocator::ChunkHandle h = (*citer);
      OriginalBFCAllocator::Chunk* chunk = ChunkFromHandle(h);
      DCHECK(!chunk->in_use());
...
void OriginalBFCAllocator::SplitChunk(OriginalBFCAllocator::ChunkHandle h, size_t num_bytes) {
  ...
  OriginalBFCAllocator::Chunk* new_chunk = ChunkFromHandle(h_new_chunk);
  ...
  OriginalBFCAllocator::ChunkHandle h_neighbor = c->next;
  new_chunk->prev = h;
...
void OriginalBFCAllocator::DeallocateRaw(void* ptr) {
  DeallocateRawInternal(ptr);
  retry_helper_.NotifyDealloc();
}

void OriginalBFCAllocator::DeallocateRawInternal(void* ptr) {
  printf("OriginalBFCAllocator::Deallocate_Raw_Internal\n");
  if (ptr == nullptr) {
  ...
  OriginalBFCAllocator::ChunkHandle h = region_manager_.get_handle(ptr);
...
void OriginalBFCAllocator::Merge(OriginalBFCAllocator::ChunkHandle h1,
                         OriginalBFCAllocator::ChunkHandle h2) {
  ...
  OriginalBFCAllocator::ChunkHandle h3 = c2->next;
  c1->next = h3;
  CHECK(c2->prev == h1);
  if (h3 != kInvalidChunkHandle) {
    OriginalBFCAllocator::Chunk* c3 = ChunkFromHandle(h3);
    c3->prev = h1;
  }
...
void OriginalBFCAllocator::DeleteChunk(ChunkHandle h) {
...
void OriginalBFCAllocator::InsertFreeChunkIntoBin(OriginalBFCAllocator::ChunkHandle h) {
  Chunk* c = ChunkFromHandle(h);
  CHECK(!c->in_use() && (c->bin_num == kInvalidBinNum));
...
void OriginalBFCAllocator::RemoveFreeChunkIterFromBin(
    OriginalBFCAllocator::Bin::FreeChunkSet* free_chunks,
    const OriginalBFCAllocator::Bin::FreeChunkSet::iterator& citer) {
  ChunkHandle h = *citer;
  Chunk* c = ChunkFromHandle(h);
  CHECK(!c->in_use() && (c->bin_num != kInvalidBinNum));
...
void OriginalBFCAllocator::RemoveFreeChunkFromBin(OriginalBFCAllocator::ChunkHandle h) {
  Chunk* c = ChunkFromHandle(h);
  CHECK(!c->in_use() && (c->bin_num != kInvalidBinNum));
...
void OriginalBFCAllocator::FreeAndMaybeCoalesce(OriginalBFCAllocator::ChunkHandle h) {
  Chunk* c = ChunkFromHandle(h);
  CHECK(c->in_use() && (c->bin_num == kInvalidBinNum));
...
bool OriginalBFCAllocator::TracksAllocationSizes() { return true; }

size_t OriginalBFCAllocator::RequestedSize(const void* ptr) {
  mutex_lock l(lock_);
  OriginalBFCAllocator::ChunkHandle h = region_manager_.get_handle(ptr);
  CHECK(h != kInvalidChunkHandle)
...
size_t OriginalBFCAllocator::AllocatedSize(const void* ptr) {
  mutex_lock l(lock_);
  OriginalBFCAllocator::ChunkHandle h = region_manager_.get_handle(ptr);
  CHECK(h != kInvalidChunkHandle)
...
int64 OriginalBFCAllocator::AllocationId(const void* ptr) {
  mutex_lock l(lock_);
  OriginalBFCAllocator::ChunkHandle h = region_manager_.get_handle(ptr);
  CHECK(h != kInvalidChunkHandle)
...
string OriginalBFCAllocator::RenderOccupancy() {
...
void OriginalBFCAllocator::DumpMemoryLog(size_t num_bytes) {
...
void OriginalBFCAllocator::GetStats(AllocatorStats* stats) {
  mutex_lock l(lock_);
  *stats = stats_;
}

void OriginalBFCAllocator::ClearStats() {
  mutex_lock l(lock_);
  stats_.num_allocs = 0;
...
std::array<OriginalBFCAllocator::BinDebugInfo, OriginalBFCAllocator::kNumBins>
OriginalBFCAllocator::get_bin_debug_info() {
