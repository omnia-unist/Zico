...
// Gangmuk
bool DirectSession::runinternal_loop_lock;
 ...
 public:
  ...
  Status NewSession(const SessionOptions& options,
    ...
    TF_RETURN_IF_ERROR(DeviceFactory::AddDevices(
		...
		// Gangmuk
    std::cout<<"@@ NewSession() is construcring a new DirectSession() object."<<std::endl;
...
// Gangmuk
int DirectSession::unique_session_id = 1;
...
DirectSession::DirectSession(const SessionOptions& options,
  ...
  // Gangmuk
  DirectSession::runinternal_loop_lock = false;
  // Gangmuk
  GM_Parser parser;

  // Gangmuk
  std::srand((unsigned int)time(0));
  int random_id = std::rand() % 1000;
  local_session_id = unique_session_id++;
  printf("@@ DirectSession() ctor, random_id: %d, local_session_id: %d\n",
         random_id, local_session_id);

  // Gangmuk
  local_step_id_counter_ = 1;
  ...
  for (auto d : device_mgr_->ListDevices()) {
    ...
    // 이제 세션을 막 여는 거니까, Item 객체를 만드는 걸 콜하게 될거다.
    // 그리고 컨스트럭터에 의해 num_holds는 1로 초기화 될 것 이다.
...
Status DirectSession::Create(const GraphDef& graph) {
  printf("@@ DirectionSession::Create(graph)\n");
...
Status DirectSession::RunInternal(int64 step_id, const RunOptions& run_options,
                                  ...
                                  RunMetadata* run_metadata) {
  printf("@@ RunInternal(local_session_id: %d, step_id: %d)\n", local_session_id, step_id);
  ...
  if (executors_and_keys->collective_graph_key !=
    ...
    if (!collective_executor_mgr_) {
      //std::cout<<"@@ !collective_executor_mgr_"<<std::endl;
    ...
    //std::cout<<"@@ run_state.collective_executor.reset(new CollectiveExecutor::Handle("<<std::endl;
  ...
  std::cout << "@@ num_executors: " << num_executors
            << ", local session id: " << local_session_id << std::endl;
  ...
  if (options_.config.graph_options().build_cost_model() > 0) {
    std::cout<<"@@ config.graph_options().build_cost_model() is true > 0"<<std::endl;
  ...
  ////////////////////////////////////////////////////////////////////////////////
  // 여기서 for문으로 돌면서 executor"들"을 다 launch하는 듯 하다.
  // printf("@@ Before for (const auto& item : executors_and_keys->items)\n");
  for (const auto& item : executors_and_keys->items) {
    // printf("@@ Inside for (const auto& item : executors_and_keys->items),%s\n",item.device->name().c_str());
    ...
    if (!device_thread_pool) {
      // 시피유는 여기로 오고,
      // printf("Use default_runner. local session id: %d\n", local_session_id);
...
Status DirectSession::Run(const RunOptions& run_options,
                          ...
                          RunMetadata* run_metadata) {
  const uint64 start_time_usecs = Env::Default()->NowMicros();
  if (print_DS) printf("@@ DirectSession::Run\n");
  TF_RETURN_IF_ERROR(CheckNotClosed());
  ...
  // Gangmuk
  int64 step_id = local_step_id_counter_.fetch_add(1);

  if (LogMemory::IsEnabled()) {
    LogMemory::RecordStep(step_id, run_state_args.handle);
  }
  
  /////////////DirectSession::Run's RunInternal /////////////////////
  ...
  uint64 elapsed = Env::Default()->NowMicros() - start_time_usecs;
  printf("DirectSession::Run,step_id,%d,ElapsedTime(ms),%llu\n", step_id, elapsed / 1000);
  return Status::OK();
}
...
Status DirectSession::PRunSetup(const std::vector<string>& input_names,
  ...
  // Gangmuk
  args.step_id = local_step_id_counter_.fetch_add(1);
  ...
  for (auto& item : executors_and_keys->items) {
    printf("@@ DirectSession::PRunSetup-%d\n", i++);
    item.executor->RunAsync(args, barrier->Get());
  }
...
Status DirectSession::CreateExecutors(
  ...
  std::cout<<"@@ CreateExecutors()"<<std::endl;
  // 빌드그래프옵션 인스턴스가 처음 생기는 부분
  ...
  for (auto iter = graphs.begin(); iter != graphs.end(); ++iter) {
    ...
    // printf("DirectSession::CreateExecutors,ek->items.resize\n"); // 왜 resize를 하는 거지... 암튼 뭐 내가 reserve와 resize를 제대로 이해를 못하고 있는 거 같다.
...
Status DirectSession::GetOrCreateExecutors(
  ...
  const string key = strings::StrCat(
      ...
      local_session_id); // local_session_id is added by gangmuk;
  ...
  // See if we already have the executors for this run.
  {
    ...
    if (it != executors_.end()) {
      *executors_and_keys = it->second.get();
      // printf("@@ executor already exists: %s\n", key.c_str());
      // for ( auto iter : executors_) {
      //   std::cout << "executor: " << iter.first << std::endl;
      // }
      return Status::OK();
    }
  }
  ...
  // See if we already have the executors for this run.
  // 이미 Executor가 있는지 보고 있으면 Get Executor해서 return
  {
    mutex_lock l(executor_lock_);
    ...
    if (it != executors_.end()) {
      *executors_and_keys = it->second.get();
      ...
      printf("@@ executor already exists-2: %s\n", key.c_str());
      return Status::OK();
    }
  }

  // 없어서 새로 Executor를 Create하려고 하는 부분.
...
Status DirectSession::CreateGraphs(
  ...
  std::cout<<"@@ CreateGraphs()"<<std::endl;
  mutex_lock l(graph_state_lock_);
  ...
  if (options_.config.graph_options().place_pruned_graph()) {
    if (print_DS)
      std::cout << "@@ place_pruned_graph() is true: "
    ...
    if(print_DS) std::cout<< "@@ place_pruned_graph() is false: " <<options_.config.graph_options().place_pruned_graph()<<std::endl;
  ...
  *collective_graph_key = client_graph->collective_graph_key;
  ...
  ////////////////////// Partition /////////////////////////////////////
  //////////////////////////////////////////////////////////////////////
  if(print_DS) std::cout<<"@@ CreateGraphs() is calling Partition()"<<std::endl;
  ...
  // partition.first = partition_name
  // partition.second = Graph
  for (const auto& partition : partitions) {
    ...
    ///////////////////////////////////////////////////////////////////////
    ///////////////ConvertGraphDefToGraph After Partitioning///////////////
    ///////////////////////////////////////////////////////////////////////
    if(print_DS) std::cout << "CreateGraphs() is calling ConvertGraphDefToGraph() After Partitioning" << std::endl;
