...
class RenamedDevice : public Device {
 public:
  ...
  /**************** Gangmuk ****************/
  size_t NumUsedEvents() const override {
    return underlying_->NumUsedEvents();
  }
  size_t NumPendingEvents() const override {
    return underlying_->NumPendingEvents();
  }
  // bool SkipAllTest() const override { 
  //   return underlying_->SkipAllTest(); 
  // }
  // bool SkipRefCountIsOne() const override {
  //   return underlying_->SkipRefCountIsOne();
  // }
  int RecordOutputTensorOnly() const override {
    return underlying_->RecordOutputTensorOnly();
  }
  // bool TensorRecordingOriginalVersion() const override {
  //   return underlying_->TensorRecordingOriginalVersion();
  // }
  // bool ThrottleKernelLaunch() const override {
  //   return underlying_->ThrottleKernelLaunch();
  // }
  int MaxLaunchedKernels() const override {
    return underlying_->MaxLaunchedKernels();
  }
  int ForwardInputProfilingMode() const override {
    return underlying_->ForwardInputProfilingMode();
  }
  int PrintForwardInputOrAllocateOutput() const override {
    return underlying_->PrintForwardInputOrAllocateOutput();
  }
  int PrintCoordinator() const override {
    return underlying_->PrintCoordinator();
  }
  std::string CoordinatorPolicy() const override {
    return underlying_->CoordinatorPolicy();
  }
  int PrintRawInternalCsv() const override {
    return underlying_->PrintRawInternalCsv();
  }

  int InplaceProfilingStep() const override {
    return underlying_->InplaceProfilingStep();
  }
  int BFCPhaseTwoStep() const override {
    return underlying_->BFCPhaseTwoStep();
  }
  int AllocationInfoRegistrationStep() const override {
    return underlying_->AllocationInfoRegistrationStep();
  }
  int PrintSummaryStep() const override {
    return underlying_->PrintSummaryStep();
  }
  int StartAligningStep() const override {
    return underlying_->StartAligningStep();
  }
  int NumVirtualGPU() const override {
    return underlying_->NumVirtualGPU();
  }
  bool TurnOnCoordinator() const override {
    return underlying_->TurnOnCoordinator();
  }
  int ForceNoInplace() const override { return underlying_->ForceNoInplace(); }
  int PrintForwardInput() const override {
    return underlying_->PrintForwardInput();
  }
  /*******************************************/
