...
class ExecutorImpl : public Executor {
 public:
  ExecutorImpl(const LocalExecutorParams& p, std::unique_ptr<const Graph> g)
    ...
    executorimpl_id = next_executorimpl_id;
    // printf("ExecutorImpl Constructor, executorimpl_id: %d\n", executorimpl_id);
    next_executorimpl_id++;
  }

  int executorimpl_id;
  static int next_executorimpl_id;
 ...
 private:
  ...
  // Gangmuk
  int max_launched_kernels_ = 0;
  bool forward_input_profiling_mode_ = false;
  // int print_coordinator = 0;
  int inplace_profiling_step = 10;
  int start_aligning_step = 14;
  bool turn_on_coordinator;
...
int ExecutorImpl::next_executorimpl_id;
...
Status ExecutorImpl::Initialize() {
  ...
  // Gangmuk
  max_launched_kernels_ = params_.device->MaxLaunchedKernels();
  forward_input_profiling_mode_ = params_.device->ForwardInputProfilingMode();
  // print_coordinator = params_.device->PrintCoordinator();
  inplace_profiling_step = params_.device->InplaceProfilingStep();
  start_aligning_step = params_.device->StartAligningStep();
  num_virtual_gpu = params_.device->NumVirtualGPU();
  turn_on_coordinator = params_.device->TurnOnCoordinator();
...
class ExecutorState {
 public:
  ...
  // Gangmuk
  bool ask_to_coordinator = false;
  // int executorstate_id = -1;
  // static int next_executorstate_id;
 ...
 private:
  ...
  class TaggedNodeReadyQueue {
   public:
    ...
    // originall ready_ variable belongs to private
  ...
  // Gangmuk: remove const to change impl_->device_record_tensor_accesses_ value in ExecutorState::Process() function
...
// Gangmuk
// int ExecutorState::next_executorstate_id;
...
ExecutorState::ExecutorState(const Executor::Args& args, ExecutorImpl* impl)
  ...
  // Gangmuk
  // executorstate_id = next_executorstate_id;
  // printf("ExecutorState Constructor, executorstate_id: %d\n", executorstate_id);
  // next_executorstate_id++;
...
void ExecutorState::Process(TaggedNode tagged_node, int64 scheduled_nsec) {
  ...
  // Gangmuk
  if (impl_->forward_input_profiling_mode_ && step_id_ >= (impl_->inplace_profiling_step + 1) ) {
      impl_->device_record_tensor_accesses_ = true;
  }
  ...
  // Gangmuk
  params.node_id = tagged_node.node->id();
  // params.executorstate_id = executorstate_id;
  params.executorimpl_id = impl_->executorimpl_id;
  ...
  while (!inline_ready.empty()) {
    // 실제로 런치하게 되는 노드를 꺼내는 부분
    tagged_node = inline_ready.front();

    if (impl_->max_launched_kernels_ > 0) {
      int max_kernel_sleep_us = 10;
      int total_max_kernel_sleep_us = 0;
      while(true) {
        size_t num_used_events = impl_->params_.device->NumUsedEvents();
        size_t num_pending_events = impl_->params_.device->NumPendingEvents();
        if (num_used_events <= impl_->max_launched_kernels_) {
          break;
        }
        Env::Default()->SleepForMicroseconds(max_kernel_sleep_us);
        total_max_kernel_sleep_us += max_kernel_sleep_us;
      }
      // if (step_id_ == 40 || step_id_ == 55 || step_id_ == 150 || step_id_ == 160) {
      //   printf("step_id,%d,node_id,%d,node_name,%s,time,%llu,total_max_kernel_sleep_us,%d\n",step_id_, tagged_node.node->id(), tagged_node.node->name().c_str(), Env::Default()->NowMicros(), total_max_kernel_sleep_us);
      // }
    }
    // if (step_id_ == 40 || step_id_ == 55 || step_id_ == 150 || step_id_ == 160) {
    //   size_t num_pending_events = impl_->params_.device->NumPendingEvents();
    //   printf("step_id,%d,node_id,%d,node_name,%s,time,%llu,num_pending_events,%zu\n", step_id_, tagged_node.node->id(), tagged_node.node->name().c_str(), Env::Default()->NowMicros(), num_pending_events);
    // }

    ///////////////////////////////////////////////////
    // gradient node // Gangmuk
    params.is_gradient = tagged_node.node->is_gradient;
    params.from_which_node = std::to_string(tagged_node.node->having_bridge);
    ///////////////////////////////////////////////////

    // if (step_id_ == 19 || step_id_ == 20) {
    //   printf("Process,%d,%d,%s\n", step_id_, tagged_node.node->id(), tagged_node.node->name().c_str());
    // }

    // Allocator* allocator_ptr = device->GetAllocator(AllocatorAttributes());
    // std::string allocator_name = allocator_ptr->Name();
    // if (step_id_ >= impl_->start_aligning_step && impl_->turn_on_coordinator && BFCAllocator::cr_global_pool_.num_allocators > 1) {
    //   // if (CoordinatorWrapper::coordinator_.all_ready == false) {
    //   //   ++CoordinatorWrapper::coordinator_.num_ready_gpu;
    //   //   printf("Executor,new_arrival,num_ready_gpu(%d), num_virtual_gpu(%d)\n",
    //   //          CoordinatorWrapper::coordinator_.num_ready_gpu,
    //   //          impl_->num_virtual_gpu);
    //   //   fflush(stdout);
    //   //   while (true) {
    //   //     // 왜 저 num_virtual_gpu가 계속 0으로 뜰까.
    //   //     if (CoordinatorWrapper::coordinator_.num_ready_gpu == impl_->num_virtual_gpu) {
    //   //       CoordinatorWrapper::coordinator_.all_ready = true;
    //   //       printf("Executor,all_ready.\nLet's go!\n");
    //   //       fflush(stdout);
    //   //       break;
    //   //     }
    //   //   }
    //   // }
    //   CoordinatorWrapper::coordinator_.Coordination(step_id_, tagged_node.node->id(), tagged_node.node->name(), device);
    // }
...
// Two parameters
// "ready" is for expensive node. "ready" queue has nodes which is ready to be run!
// "inline_ready" is for inexpensive node.
