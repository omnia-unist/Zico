...
GraphExecutionState::GraphExecutionState(
  ...
  // placement option.
  virtual_gpu = parser.parse_target_config("NumVirtualGPU");
}
...
/* static */ Status GraphExecutionState::MakeForBaseGraph(
  ...
  std::cout << "@@ GraphExecutionState::MakeForBaseGraph()" << std::endl;
  ...
  if (!ret->session_options_->config.graph_options().place_pruned_graph()) {
    ...
    // if (print_GES) std::cout<<"@@ MakeForBaseGraph() is calling InitBaseGraph()"<<std::endl;
...
/* static */ Status GraphExecutionState::MakeForPrunedGraph(
  ...
  std::cout << "@@ MakeForPrunedGraph()" << std::endl;
...
Status GraphExecutionState::Extend(
  ...
  GraphDef gdef;
  if (print_GES) std::cout << "@@ Extend()" << std::endl;
...
Status GraphExecutionState::PruneGraph(
  ...
  // feed와 fetch를 각각 Arg와 Retval로 rewrite하는 과정으로 보인다.
  // 무슨 의미인지는 아직 잘 모르겠다.
  ...
  // in config.proto file
  // Tensors to be connected in the callable. Each TensorConnection denotes
  // a pair of tensors in the graph, between which an edge will be created
  // in the callable.
  // repeated TensorConnection tensor_connection = 5;

  // CallableOption은 subgraph를 위한 option이다.
  // CallableOption안에 TensorConnection이 있고
  // TensorConnection은 subgraph의 from_node와 to_node가 있다.
  ...
  ///////////////////////////////////////////////////////////////
  ////////////////////RewriteGraphFroExecution///////////////////
  ///////////////////////////////////////////////////////////////
  if (print_GES) std::cout<<"@@ PruneGraph is calling RewriteGraphForExecution "<<std::endl;
...
Status GraphExecutionState::InitBaseGraph(const BuildGraphOptions& options) {
  std::cout << "@@ GraphExecutionState::InitBaseGraph()" << std::endl;
  const GraphDef* graph_def = &original_graph_def_;

  std::unique_ptr<Graph> new_graph(new Graph(OpRegistry::Global()));
//  new_graph->unique_graph_id = -1*(new_graph->unique_graph_id);
//  printf("InitBaseGraph, new_graph->unique_graph_id: %d\n",
//         new_graph->unique_graph_id);
...
#ifndef IS_MOBILE_PLATFORM
  if (print_GES) std::cout << "@@ OptimizeGraph" << std::endl;
  if (session_options_->config.graph_options().place_pruned_graph()) {
    return errors::InvalidArgument("Can't optimize a pruned graph");
  }

  if (grappler::MetaOptimizerEnabled(session_options_->config)) {
    if (print_GES) std::cout<<"@@ grappler::MetaOptimizerEnabled(session_options_->config)"<<std::endl;
    ...
    if (!(options.callable_options.feed().empty() &&
      ...
      for (const string& feed : options.callable_options.feed()) {
        ...
        if (print_GES) std::cout<< "@@ OptimizeGraph/feed: " <<feed<<std::endl;
      }
      for (const TensorConnection& tensor_connection :
           options.callable_options.tensor_connection()) {
...
Status GraphExecutionState::BuildGraph(const BuildGraphOptions& options,
  ...
  if (session_options_ == nullptr ||
    ...
    ////////////////////////PruneGraph/////////////////////////////////
    ///////////////////////////////////////////////////////////////////
    std::cout<<"@@ BuildGraph is calling PruneGraph()"<<std::endl;
    TF_RETURN_IF_ERROR(
    ...
    // a member variable.
    if (print_GES) std::cout<<"@@ graph was already pruned(?) when it was constructed"<<std::endl;
  ...
  // RunGrouping이 뭘 하는 건지 아직 잘 모르겠다.
  // std::cout<<"@@ calling OptimizationPassRegistry::Global()->RunGrouping()"<<std::endl;
  TF_RETURN_IF_ERROR(OptimizationPassRegistry::Global()->RunGrouping(
      OptimizationPassRegistry::POST_REWRITE_FOR_EXEC, optimization_options));

  // std::cout<<"@@ option.collective_graph_key: "<<options.collective_graph_key<<std::endl;
  ...
  if (collective_graph_key == BuildGraphOptions::kNoCollectiveGraphKey) {
    ...
    std::set<int32> instance_key_set;
		bool print_optimized_graph = false;
    //std::cout<<"@@ Print All the Nodes in Optimized_Graph"<<std::endl;
    for (Node* node : optimized_graph->nodes()) {
      if (node->IsCollective()) {
//				if(print_optimized_graph){
//					std::cout<<"@@ found collective operation!!!"<<std::endl;
//					std::cout<<"@@ node->name(): "<<node->name()<<std::endl;
//					std::cout<<"@@ node->id(): "<<node->id()<<std::endl;
//					std::cout<<"@@ node->type_string(): "<<node->type_string()<<std::endl;
//				}
        int32 instance_key;
        TF_RETURN_IF_ERROR(
