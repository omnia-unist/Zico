...
BFCAllocator::BFCAllocator(SubAllocator* sub_allocator, size_t total_memory,
  ...
  tighten_ar = false;
  default_ar_size = 256;  // meaningless because tightne_ar is false
  default_ar_size = default_ar_size << 20;
  // bfc_phase_two_step = 99999;  // meaningless because adaptive_alloc is false
  start_aligning_step = 99999;  // meaningless because adaptive_alloc is false
...
BFCAllocator::BFCAllocator(SubAllocator* sub_allocator, size_t total_memory,
  ...
  print_bfc_overhead = false;
  // print_bfc_overhead = parser.parse_target_config("PrintBFCOverhead");
  ...
  two_bin = parser.parse_target_config("TwoBin");
  ...
  if (Name() == "GPU_0_bfc") {
    ...
    CHECK_EQ(0,1);
    printf("GANGMUK_ERROR: %s is not supported BFC Name\n", Name().c_str());
  }
  ...
  std::cout << "BFCRecord,StepId,FunctionName,Allocator,ChunkSize,"
            << "IndividualChunkLevelBytes,AggChunkLevelBytes,"
               "IndividualRegionLevelBytes,AggRegionLevelBytes,"
            << "Time,IsGradient,Sharable,LifeSpan,Caller,KernelName"
            << std::endl;
}
...
BFCAllocator::~BFCAllocator() {
  ...
  for (const auto& region : region_manager_.regions()) {
    ...
    cr_global_pool_.RecordCUDAFree(name_, current_step_id, region.memory_size(), true);
...
void BFCAllocator::PrintOverhead(string caller, size_t size, uint64 elapsed) {
  if ((current_step_id == print_bfc_overhead_step || current_step_id == print_bfc_overhead_step+10 || current_step_id == print_bfc_overhead_step+20)) {
...
void BFCAllocator::RegisterNewAllocationRegion(size_t bytes, void* mem_addr, size_t region_id,
  ...
  region_manager_.AddAllocationRegion(mem_addr, bytes, region_id, sharable_, from_gp);
  // Create one large chunk for the whole memory space that will
  // be chunked later.
  ChunkHandle h = AllocateChunk();
  printf("ChunkHandle h = AllocateChunk();\n");
  fflush(stdout);
  ...
  printf("InsertFreeChunkIntoBin(h, sharable_);\n");
  fflush(stdout);
}
...
bool BFCAllocator::Extend(size_t alignment, size_t rounded_bytes,
  ...
  */
  bool sharable_ = GetSharableFromNodeInfo(node_info);
  if (Name() != "cuda_host_bfc" && activate_global_pool && sharable_) {
    if (!adaptive_alloc || (adaptive_alloc && phase_two)) {
      if (debug_print) {
      ...
      size_t minimum_required_region_size = default_ar_size;
      while (rounded_bytes > minimum_required_region_size) {
        minimum_required_region_size *= 2;
      }
      ret_free_ar = cr_global_pool_.AllocRegionFromGlobalPool(rounded_bytes, minimum_required_region_size, name_, 
                                                              current_step_id, elapsed, sharable_, 
                                                              total_region_allocated_bytes_, bfc_id);
      if (debug_print) {
  ...
  */
  ...
  sub_allocator_mtx.unlock();
  cr_global_pool_.RecordCUDAAlloc(name_, current_step_id, bytes, sharable_);
...
void* BFCAllocator::AllocateRaw(size_t unused_alignment, size_t num_bytes) {
  ...
  if (print_bfc_overhead) {
    PrintOverhead("AllocOutsideOverhead", chunk->size, elapsed);
  }
  if (r != nullptr) {
    if(debug_print == true){
      printf("2 argument AllocateRaw without node_info returns with valid pointer\n");
      fflush(stdout);
    }
    return r;
  } else {
    if(debug_print == true){
      printf("2 argument AllocateRaw fails, wait 10 second and retry...\n");
      fflush(stdout);
      CHECK_EQ(0,1);
    }
...
void* BFCAllocator::AllocateRaw(size_t unused_alignment, size_t num_bytes, 
  ...
  if(debug_print == true){
    printf("++ 2 arguments - AllocateRaw(bytes: %d) with node_info - if(no_retry) is calling AllocateRawInternal(bytes: %d)\n", num_bytes, num_bytes);
    fflush(stdout);
  }
  const uint64 start_time_nsecs = Env::Default()->NowNanos();
...
void* BFCAllocator::AllocateRaw(size_t unused_alignment, size_t num_bytes,
  ...
  if (allocation_attr.no_retry_on_failure) {
    ...
    bool dump_log_on_failure = VLOG_IS_ON(2);
    if(debug_print == true){
      printf("++ 3 arguments - AllocateRaw(bytes: %d) with node_info - if(no_retry) is calling AllocateRawInternal(bytes: %d)\n", num_bytes, num_bytes);
      fflush(stdout);
    }
...
void* BFCAllocator::AllocateRawInternal(size_t unused_alignment,
  ...
  if (Extend(unused_alignment, rounded_bytes, dummy_node_info)) {
    ...
    if (ptr != nullptr) {
      if (debug_print){
        printf("AllocateRawInternal(original-2nd) return valid pointer\n");
        fflush(stdout);
      }
      return ptr;
    } 
...
// Gangmuk: Newly Added 3
// 3rd, third
void* BFCAllocator::AllocateRawInternal(size_t unused_alignment, size_t num_bytes, bool dump_log_on_failure, 
                                        std::map<std::string, std::string> node_info) {
  if (num_bytes == 0) {
    LOG(ERROR) << "tried to allocate 0 bytes";
    return nullptr;
  ...
  // printf("before stoi(%s) - 0\n", node_info["step_id"].c_str()); // debug
  ...
  if (current_step_id != step_id) {
    ...
    if (current_step_id >= start_aligning_step && phase_two == false) {
      phase_two = true;
  ...
  if (ptr != nullptr) {
    ...
    if (Name().find("GPU") != std::string::npos) {
      BFCAllocator::ChunkHandle h = region_manager_.get_handle(ptr);
      Chunk* c = ChunkFromHandle(h);
      cr_global_pool_.RecordAllocationInfo(current_step_id, bfc_id, c->size, c->is_gradient, c->sharable, stats_.bytes_in_use, total_region_allocated_bytes_, node_info);
      cr_global_pool_.SimulateIfNewStep(bfc_id, current_step_id);
    }
    if (debug_print) {
      printf(
          "++ Success new3-1st FindChunkPtr(bin_num: %d, rounded_bytes: %d, "
          "num_bytes: %d, sharable: %s)\n",
          bin_num, rounded_bytes, num_bytes,
  ...
  if (debug_print){ printf("!! new3-1st FindChunkPtr FAIL!! -> calling Extend\n"); }

  if (Extend(unused_alignment, rounded_bytes, node_info)) {
    if (debug_print){ printf("++ After Extend, AllocateRawInternal(new3-2nd) is trying FindChunkPtr\n"); }
    // FindingChunkPtr 2nd Trial
		if (debug_print) {
			// FindChunkPtr,bin_num,rounded_bytes,sharable,start,end
			printf("FindChunkPtr-2nd,%d,%zu,%s,%d\n", 
              bin_num, rounded_bytes, SharableToString(GetSharableFromNodeInfo(node_info)).c_str(), getTimeDiff());
		}
    ...
    if (debug_print){ printf("  ++ After finishing 2nd FindChunkPtr(%d, %d, %d,)\n", bin_num, rounded_bytes, num_bytes); }
    if (ptr != nullptr) {
      RecordNodeInfoToChunk(ptr, node_info);
      if (Name().find("GPU") != std::string::npos) {
        BFCAllocator::ChunkHandle h = region_manager_.get_handle(ptr);
...
// Gangmuk: ACT
void* BFCAllocator::FindChunkPtr(BinNum bin_num, size_t rounded_bytes, size_t num_bytes,
                                 std::map<std::string, std::string> node_info) {
  const uint64 start_time_nsecs = Env::Default()->NowNanos();
    ...
    for (auto citer = b->free_chunks.begin(); citer != b->free_chunks.end(); ++citer) {
      ...
      if (chunk->size >= rounded_bytes) {
        ...
        // It must be always true
          ...
          //       Name(), chunk->size, stats_.bytes_in_use, current_step_id,
          ...
          printf("FindChunkPtr valid chunk ptr\n");
          fflush(stdout);
          return chunk->ptr;
        }
				else {
  ...
  printf("FindChunkPtr nullptr return\n");
  fflush(stdout);
  return nullptr;
}
...
void BFCAllocator::DeallocateRaw(void* ptr) {
  ...
  retry_helper_.NotifyDealloc();
   uint64 elapsed = Env::Default()->NowNanos() - start_time_nsecs;
  if (print_bfc_overhead) {
    PrintOverhead("DeallocOutsideOverhead", -1, elapsed);
  }
  // 버그 유발 지점 입니다. Dealloc하고 해당 청크를 refer하려고 하면 안됩니다.
...
void BFCAllocator::DeallocateRawInternal(void* ptr) {
  ...
  if (Name().find("GPU") != std::string::npos) {
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////
    cr_global_pool_.RecordDeallocationInfo(current_step_id, bfc_id, c->size, c->is_gradient, c->sharable, lifespan, stats_.bytes_in_use, total_region_allocated_bytes_, c->kernel_name, c->caller);
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////
  }
  ...
  uint64 elapsed = Env::Default()->NowNanos() - start_time_nsecs;
  if (print_bfc_overhead) {
    PrintOverhead("DeallocInsideOverhead", -1, elapsed);
  }
    ...
    if (adaptive_alloc) {
      ...
      if (!activate_global_pool) {
        ...
        if (!phase_two) {
          ...
          cr_global_pool_.RecordCUDAFree(name_, current_step_id, AR_size, c->sharable);
          sub_allocator_mtx.unlock();
