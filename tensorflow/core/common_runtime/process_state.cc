...
Allocator* ProcessState::GetCPUAllocator(int numa_node) {
  ...
  while (cpu_allocators_.size() <= static_cast<size_t>(numa_node)) {
    ...
    Allocator* allocator = nullptr;
		printf("********* Calling new SubAllocator Constructor for CPU\n");
    ...
    if (use_bfc_allocator) { // 여기로 안들어옴.
      ...
      printf("********* Calling CPU BFCAllcator Constructor for \n");  // 출력 안됨.
      allocator =
          new BFCAllocator(sub_allocator, cpu_mem_limit, true /*allow_growth*/,
                           "bfc_cpu_allocator_for_gpu" /*name*/);
      // Gangmuk
      // allocator = new OriginalBFCAllocator(
      //     sub_allocator, cpu_mem_limit, true /*allow_growth*/,
      //     "bfc_cpu_allocator_for_gpu" /*name*/);
      ...
      VLOG(2) << "Using PoolAllocator for ProcessState CPU allocator "
              ...
              << " numa_node=" << numa_node;
      printf("@@ PoolAllocator for CPU Allocator\n");
    } else {
      // 여기로 들어온다.
      DCHECK(!sub_allocator);
      allocator = cpu_allocator();
      printf("@@ cpu_allocator() for CPU Allocator\n"); // 이거 출력됨.
    }
