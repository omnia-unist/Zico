...
Allocator* GPUProcessState::GetGPUAllocator(const GPUOptions& options,
  ...
  if (allocator_parts.allocator.get() == nullptr) {
    ...
    // Gangmuk: manually turn on uvm(use_unified_memory) forcefully.
    // bool uvm_on = (options.per_process_gpu_memory_fraction() > 1.0 || options.experimental().use_unified_memory());
    bool uvm_on = true;
    ...
    GM_Parser parser;
    bool one_allocator = parser.parse_target_config("OneAllocator");
    Allocator* gpu_allocator = nullptr;
    if (one_allocator) {
      /////////////
      // Gangmuk //
      ////////////////////////////////////////////////////////////////////////////
      // GPU_1가 GPU_0의 GPUBFCAllocator를 쓰게 만들기 위해 내가 추가한 코드
      if (tf_gpu_id.value() == 0) {
        // Same as original code
        printf("GetGPUAllocator,GPU_0_bfc is created. (tf_gpu_id: %d)\n", tf_gpu_id.value());
        fflush(stdout);
        gpu_allocator = new GPUBFCAllocator(sub_allocator, total_bytes, options, strings::StrCat("GPU_", tf_gpu_id.value(), "_bfc"));
      } else {
        printf("GetGPUAllocator,GPU_0_bfc is returned. (tf_gpu_id: %d)\n", tf_gpu_id.value());
        fflush(stdout);
        AllocatorParts& allocator_parts = gpu_allocators_[0];
  ...
  if (process_state_->ProcessState::FLAGS_brain_gpu_record_mem_types) {
    // printf("allocator_parts.recording_allocator.get();\n");
    return allocator_parts.recording_allocator.get();
  } else {
    // printf("return allocator_parts.allocator.get();\n");
    return allocator_parts.allocator.get();
  }
#else
