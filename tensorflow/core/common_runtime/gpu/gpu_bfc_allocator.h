...
class GPUMemAllocator : public SubAllocator {
 public:
  ...
  explicit GPUMemAllocator(se::StreamExecutor* stream_exec,
    ...
    printf("************ GPUMemAllcator() Constructor for SubAllocator for GPU memory\n");
  }
  ~GPUMemAllocator() override {}

	// Gangmuk
	static std::mutex gpu_bfc_allocate_mtx;
	static std::mutex gpu_bfc_deallocate_mtx;
  ...
  void* Alloc(size_t alignment, size_t num_bytes) override {
    ...
    if (num_bytes > 0) {
      if (use_unified_memory_) {
        // printf("GPUMemAllocator, Alloc, use_unified_memory_\n");
        ptr = stream_exec_->UnifiedMemoryAllocate(num_bytes);
        // ptr = stream_exec_->UnifiedMemoryAllocate(num_bytes, gpu_bfc_allocate_mtx);
      } else {
        // printf("GPUMemAllocator, Alloc, \"no\" use_unified_memory_\n");
  ...
  void Free(void* ptr, size_t num_bytes) override {
    ...
    if (ptr != nullptr) {
      ...
      if (use_unified_memory_) {
        printf("GPUMemAllocator, Free, use_unified_memory_\n");
        stream_exec_->UnifiedMemoryDeallocate(ptr);
        // stream_exec_->UnifiedMemoryDeallocate(ptr, gpu_bfc_deallocate_mtx);
      } else {
        printf("GPUMemAllocator, Free, \"no\" use_unified_memory_\n");
