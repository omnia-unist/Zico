...
BaseGPUDevice::BaseGPUDevice(const SessionOptions& options, const string& name,
  ...
  /******************** Gangmuk ********************/
  // recording_tensors =
  //     options.config.gpu_options().gangmuk().recording_tensors();
  recording_tensors = parser.parse_target("TensorRecording");

  // skip_all_test = options.config.gpu_options().gangmuk().skip_all_test();

  // skip_ref_count_is_one = options.config.gpu_options().gangmuk().skip_ref_count_is_one();

  // record_output_tensor_only =
  //     options.config.gpu_options().gangmuk().record_output_tensor_only();
  record_output_tensor_only = parser.parse_target("RecordOutputTensorOnly");

  // tensor_recording_original_version =
  //     options.config.gpu_options().gangmuk().tensor_recording_original_version();

  // throttle_kernel_launch =
  //     options.config.gpu_options().gangmuk().throttle_kernel_launch();

  // max_launched_kernels =
  //     options.config.gpu_options().gangmuk().max_launched_kernels();
  max_launched_kernels = parser.parse_target("MaxKernel");

  // forward_input_profiling_mode =
  //     options.config.gpu_options().gangmuk().forward_input_profiling_mode();
  forward_input_profiling_mode = parser.parse_target("InplaceProfiling");

  virtual_gpu = parser.parse_target("VirtualGPU");
  gpu_0_memory = parser.parse_target("GPU_0_Memory");
  gpu_1_memory = parser.parse_target("GPU_1_Memory");
  ...
  if (recording_tensors == 1 && record_output_tensor_only != 1) {
    printf(
        "TensorRecording is turned but RecordOutputTensorOnly is turned off... "
        "Are you sure??????????\n");
    exit(1);
  }

    print_forward_input_or_allocate_output =
        options.config.gpu_options()
            .gangmuk()
            .print_forward_input_or_allocate_output();

  // print_raw_internal_csv =
  //     options.config.gpu_options().gangmuk().print_raw_internal_csv();
  print_raw_internal_csv = parser.parse_target("PrintBFC");

  // print_coordinator =
  //     options.config.gpu_options().gangmuk().print_coordinator();

  // inplace_profiling_step =
  //     options.config.gpu_options().gangmuk().inplace_profiling_step();
  // bfc_phase_two_step =
  //     options.config.gpu_options().gangmuk().bfc_phase_two_step();
  // allocation_info_registration_step = options.config.gpu_options()
  //                                         .gangmuk()
  //                                         .allocation_info_registration_step();
  // print_summary_step =
  //     options.config.gpu_options().gangmuk().print_summary_step();
  // start_aligning_step =
  //     options.config.gpu_options().gangmuk().start_aligning_step();
  // force_no_inplace =
  // options.config.gpu_options().gangmuk().force_no_inplace();

  print_coordinator = parser.parse_target("PrintCoordinator");
  inplace_profiling_step = parser.parse_target("InplaceProfilingStep");
  bfc_phase_two_step = parser.parse_target("BFCPhaseTwoStep");
  allocation_info_registration_step = parser.parse_target("AllocationInfoRegistrationStep");
  print_summary_step = parser.parse_target("PrintSummaryStep");
  start_aligning_step = parser.parse_target("PrintBFC");
  force_no_inplace = parser.parse_target("ForceNoInplace");
  print_forward_input = parser.parse_target("PrintForwardInput");
  /****************************************************/
}
...
Status BaseGPUDevice::Init(const SessionOptions& options) {
  ...
  // Gangmuk: overwrite
  int parsed_gpu_thread_mode = parser.parse_target("GpuThreadMode");
  ...
  if (gpu_thread_mode != "global") {
    ...
    // Gangmuk: overwrite
    int parsed_gpu_thread_count = parser.parse_target("GpuThreadCount");
...
void BaseGPUDevice::ConsumeListOfAccessedTensors(
  ...
  se::Stream* stream = gpu_device_context->stream();
  // Gangmuk
  // printf("@@ BaseGPUDevice::ConsumeListOfAccessedTensors\n");
  em_->ThenDeleteTensors(stream, tensor_refs);
}

/********************************************************************************/
size_t BaseGPUDevice::getPendingEventsSize() const { return em_->used_events_.size(); }
// bool BaseGPUDevice::SkipAllTest() const { return skip_all_test; }
// bool BaseGPUDevice::SkipRefCountIsOne() const { return skip_ref_count_is_one; }
int BaseGPUDevice::RecordOutputTensorOnly() const { return record_output_tensor_only; }
// bool BaseGPUDevice::TensorRecordingOriginalVersion() const { return tensor_recording_original_version; }
// bool BaseGPUDevice::ThrottleKernelLaunch() const { return throttle_kernel_launch; }
...
Status BaseGPUDeviceFactory::CreateDevices(
  ...
  if (!virtual_devices.empty()) {
    ...
    printf("@@ virtual_devices.size(): %zu\n", virtual_devices.size());
  ...
  for (int i = 0; i < num_gpus_to_use; ++i) {
    ...
    // memory_limit_bytes_.size() platform gpu(visible device) 가 가지고 있는
