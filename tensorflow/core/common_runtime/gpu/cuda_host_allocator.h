    ...
    if (num_bytes > 0) {
      ...
      if (ptr == nullptr) {
        LOG(WARNING) << "could not allocate pinned host memory of size: "
                     << num_bytes;
        // Gangmuk: To kill the process forcefully.
        printf("@@ CUDAHostAllocator Alloc Fail: alignment, %zu, num_bytes, %zu\n", alignment, num_bytes);
        CHECK_EQ(1,0);
        return ptr;
      }
      else {
        printf("@@ CUDAHostAllocator Alloc: alignment, %zu, num_bytes, %zu\n",
               alignment, num_bytes);
      }
