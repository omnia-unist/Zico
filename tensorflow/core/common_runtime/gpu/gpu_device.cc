...
class BaseGPUDevice::StreamGroupFactory {
 public:
  ...
  BaseGPUDevice::StreamGroup* GetOrCreate(TfGpuId tf_gpu_id,
                                          int stream_group_within_gpu, /* max_streams_가 1이기 때문에 항상 1 */
                                          se::StreamExecutor* executor,
                                          const GPUOptions& options) {
    mutex_lock guard(lock_);
    StreamGroup* group =
        &streams_[key_type(tf_gpu_id.value(), stream_group_within_gpu)];
    // Gangmuk: 지금 내가 이해하기로는 각 tf gpu마다 BaseGPUDevice가 생기는 데
    // 왜 streams_의 key_type에서 tf_gpu_id를 쓰지? 각 tf gpu 마다
    // BaseGPUDevice가 생기는게 아닌가?

    if (!group->compute) {
      // Gangmuk
      // 새로운 스트림을 만드는 부분. executor가 cuda에선 context
      // 총 3개 + device_to_device_copy stream 갯수의 stream을 만든다. (context를 만든다?)
...
// Gangmuk
// 실질적인 CreateGPUDevice 함수가 도달하는 곳
BaseGPUDevice::BaseGPUDevice(const SessionOptions& options, const string& name,
  ...
  printf("@@ BaseGPUDevice() ctor\n");
  /******************** Gangmuk ********************/
  recording_tensors = parser.parse_target_config("TensorRecording");
  record_output_tensor_only = parser.parse_target_config("RecordOutputTensorOnly");

  max_launched_kernels = parser.parse_target_config("MaxKernel");
  forward_input_profiling_mode = parser.parse_target_config("InplaceProfiling");
  print_raw_internal_csv = parser.parse_target_config("PrintBFC");
  print_coordinator = parser.parse_target_config("PrintCoordinator");
  coordinator_policy = parser.parse_string_config("CoordinatorPolicy");
  inplace_profiling_step = parser.parse_target_config("InplaceProfilingStep");
  bfc_phase_two_step = parser.parse_target_config("BFCPhaseTwoStep");
  allocation_info_registration_step = parser.parse_target_config("AllocationInfoRegistrationStep");
  // print_summary_step = parser.parse_target_config("PrintSummaryStep");
  // start_aligning_step = parser.parse_target_config("StartAligningStep");
  start_aligning_step = -1;
  num_virtual_gpu = parser.parse_target_config("NumVirtualGPU");
  turn_on_coordinator = parser.parse_target_config("TurnOnCoordinator");
  force_no_inplace = parser.parse_target_config("ForceNoInplace");
  print_forward_input = parser.parse_target_config("PrintForwardInput");
  if (recording_tensors == 1 && forward_input_profiling_mode == 1) {
    printf("TensorRecording must be turned off to use InplaceProfiling\n");
    exit(1);
  }
  if (recording_tensors == 1 && record_output_tensor_only != 1) {
    printf("[GANGMUK_WARNING]: TensorRecording is turned but RecordOutputTensorOnly is turned off... Are you sure??????????\n");
    // CHECK_EQ(0,1);
  }
  /****************************************************/
}
...
Status BaseGPUDevice::Init(const SessionOptions& options) {
  ...
  int deferred_megabytes_threshold = parser.parse_target_config("DeferredMegaBytesThreshold");
  long long deferred_bytes_threshold = deferred_megabytes_threshold;
  deferred_bytes_threshold = deferred_bytes_threshold << 20;
  printf("deferred_bytes_threshold: %lld\n", deferred_bytes_threshold);
  int polling_active_delay_usecs = parser.parse_target_config("PollingActiveDelayUsecs");
  int low_latency_hint = parser.parse_target_config("LowLatencyHint");
  int kNumThreads = parser.parse_target_config("kNumThreads");
  em_.reset(new EventMgr(executor_, deferred_bytes_threshold,
                         polling_active_delay_usecs, low_latency_hint,
                         kNumThreads));
  ...
  // Gangmuk
  // stream을 max_streams_만큼 만들어 놓고, 첫번째 stream으로만 info를 채우네.
  // 왜냐면 max_streams_가 1로 하드코딩 되어있기 때문에, streams_ 사이즈가 하나이다. 
  // 즉, 하나 StreamGroup  밖에 안들어있다.
  ...
  // Gangmuk: overwrite
  int parsed_gpu_thread_mode = parser.parse_target_config("GpuThreadMode");
  if (parsed_gpu_thread_mode != -1) {
    if (parsed_gpu_thread_mode == 0) {
      printf("parsed_gpu_thread_mode is 0. (gpu_private)\n");
      gpu_thread_mode = "gpu_private";
    } else if (parsed_gpu_thread_mode == 1) {
      printf("parsed_gpu_thread_mode is 1. (global)\n");
      gpu_thread_mode = "global";
    }
    else {
      printf(
          "GpuThreadMode must be either 0 or 1. Current GpuThreadMode value is "
          "%d\n",
          parsed_gpu_thread_mode);
      exit(-1);
    }
  }

  if (gpu_thread_mode != "global") {
    printf("TF_GPU_THREAD_MODE is Not global.\n");
    ...
    // Gangmuk: overwrite
    int parsed_gpu_thread_count = parser.parse_target_config("GpuThreadCount");
    if (parsed_gpu_thread_count > 0) {
      printf("gpu_thread_count = parsed_gpu_thread_count(%d)\n", parsed_gpu_thread_count);
      gpu_thread_count = parsed_gpu_thread_count;
    }
    printf("TF_GPU_THREAD_COUNT: %d\n", gpu_thread_count);
    if (gpu_thread_mode == "gpu_private") {
      printf("TF_GPU_THREAD_MODE is gpu_private.\n");
      ...
      printf("gpu_thread_mode is gpu_shared.\n");
      ...
      string error_message =
          strings::StrCat("Invalid TF_GPU_THREAD_MODE: ", gpu_thread_mode);
      LOG(WARNING) << error_message;
      return errors::InvalidArgument(error_message);
    }
  }
  else {
    printf("TF_GPU_THREAD_MODE is global.\n");
  }
...
void BaseGPUDevice::ComputeHelper(OpKernel* op_kernel,
  ...
  // Gangmuk: 여기서 streams_.size()는 max_streams_만큼이고 max_streams_는 계속
  // 말했듯이 '1'로 하드코딩 되어있다.
  ...
  // Gangmuk: 출력됨. 여기 지나감.
  // printf("@@ op_kernel(%s)->Compute(context) in ComputeHelper of gpu_device.cc\n", op_kernel->name().c_str());
...
bool BaseGPUDevice::RequiresRecordingAccessedTensors() const {
  ...
  if (recording_tensors) {
    return streams_.size() > 0; // always return true
  }
  // Original Code
  else {
    return streams_.size() > 1; // return false, at least it is supposed to do.
  }
}
...
/********************************************************************************/
size_t BaseGPUDevice::NumUsedEvents() const { return em_->used_events_.size(); }
size_t BaseGPUDevice::NumPendingEvents() const { return em_->num_pending_events_; }
int BaseGPUDevice::RecordOutputTensorOnly() const { return record_output_tensor_only; }
int BaseGPUDevice::MaxLaunchedKernels() const { return max_launched_kernels; }
int BaseGPUDevice::ForwardInputProfilingMode() const { return forward_input_profiling_mode; }
int BaseGPUDevice::PrintForwardInputOrAllocateOutput() const { return print_forward_input_or_allocate_output; }
int BaseGPUDevice::PrintCoordinator() const { return print_coordinator; }
std::string BaseGPUDevice::CoordinatorPolicy() const { return coordinator_policy; }
int BaseGPUDevice::PrintRawInternalCsv() const { return print_raw_internal_csv; }
int BaseGPUDevice::InplaceProfilingStep() const { return inplace_profiling_step; }
int BaseGPUDevice::BFCPhaseTwoStep() const { return bfc_phase_two_step; }
int BaseGPUDevice::AllocationInfoRegistrationStep() const { return allocation_info_registration_step; }
int BaseGPUDevice::PrintSummaryStep() const { return print_summary_step; }
int BaseGPUDevice::StartAligningStep() const { return start_aligning_step; }
int BaseGPUDevice::NumVirtualGPU() const { return num_virtual_gpu; }
bool BaseGPUDevice::TurnOnCoordinator() const { return turn_on_coordinator; }
int BaseGPUDevice::ForceNoInplace() const { return force_no_inplace; }
int BaseGPUDevice::PrintForwardInput() const { return print_forward_input; }
/********************************************************************************/
...
void BaseGPUDevice::ComputeAsync(AsyncOpKernel* op_kernel,
  ...
  se::cuda::ScopedActivateExecutorContext scoped_activation{stream->parent()};
  // printf("@@ op_kernel(%s)->ComputeAsync(context) in ComputeAsync of gpu_device.cc\n", op_kernel->name().c_str());
...
Status VerifyVirtualDeviceSettings(
    const size_t num_gpus_to_use, const GPUOptions& gpu_options,
    const std::vector<PlatformGpuId>& visible_gpu_order,
    const std::vector<PlatformGpuId>& valid_platform_gpu_ids, int num_virtual_gpu) {
  const auto& virtual_devices = gpu_options.experimental().virtual_devices();
  printf("GPUDevice,VerifyVirtualDeviceSettings, virtual_devices.size(): %zu\n",
         virtual_devices.size());
  printf("GPUDevice,VerifyVirtualDeviceSettings, num_virtual_gpu: %d\n",
         num_virtual_gpu);
  // CHECK(!virtual_devices.empty());
  CHECK_GT(num_virtual_gpu, 0);

  if (gpu_options.per_process_gpu_memory_fraction() > 0) {
    printf(
        "GPUDevice,VerifyVirtualDeviceSettings, "
        "gpu_options.per_process_gpu_memory_fraction() > 0\n");
    return errors::InvalidArgument(
  ...
  // if (num_gpus_to_use < num_virtual_gpu) {
  //   printf(
  //       "VerifyVirtualDeviceSettings, num_gpus_to_use(%d) < "
  //       "num_virtual_gpu(%d)\n",
  //       num_gpus_to_use, num_virtual_gpu);
  //   return errors::Unknown(
  //       "Gangmuk, Not enough GPUs to create virtual devices."
  //       " num_gpus_to_use: ",
  //       num_gpus_to_use, " #virtual_devices: ", virtual_devices.size());
  // }
  ...
  if (!gpu_options.visible_device_list().empty() &&
    ...
    printf(
        "VerifyVirtualDeviceSettings, "
        "!gpu_options.visible_device_list().empty() && "
        "visible_gpu_order.size() != virtual_devices.size()\n");
    return errors::InvalidArgument(
  ...
  //   printf("VerifyVirtualDeviceSettings, valid_platform_gpu_ids.size() != virtual_devices.size()\n");
  //   return errors::Unknown(
...
Status SingleVirtualDeviceMemoryLimit(const GPUOptions& gpu_options,
  ...
  // bool uvm_on = per_process_gpu_memory_fraction > 1.0 || gpu_options.experimental().use_unified_memory();// use_unfied_memory

  // Gangmuk: force uvm(use_unified_memory) on.
  bool uvm_on = true;// use_unfied_memory

  if (uvm_on) {
    // Gangmuk: I force use_unified_memory to be true. (uvm)
    printf("GPUDevice,per_process_gpu_memory_fraction > 1.0 ||"
  ...
  if (per_process_gpu_memory_fraction == 0) {
    ...
    // const int64 min_system_memory = MinSystemMemory(available_memory); // Original code
    // Gangmuk: 내가 그냥 메뉴얼 하게 넣음. 기존의 코드도 전체
    // 지피유 메모리의 5%로 그냥 min_system_memory를 잡음.
    // 이 해당 5%가 실제 보다 좀 더 큼. 혹시나 host memory를 안건들게 하기 위해
    // 보수적으로 설정을 해놓은 듯하다.
    int64 gangmuk_sys_mem = 205;
    const int64 min_system_memory = gangmuk_sys_mem * 1024 * 1024;  // Gangmuk
    if (min_system_memory < allocated_memory) {
      allocated_memory -= min_system_memory;
      printf(
          "GPUDevice,SingleVirtualDeviceMemoryLimit,min_system_memory(%lld) < "
          "allocated_memory(%lld)\n",
          min_system_memory / (1024 * 1024), allocated_memory / (1024 * 1024));
    }
  } else {
    allocated_memory = total_memory * per_process_gpu_memory_fraction;
    printf(
        "GPUDevice,min_system_memory >= "
        "allocated_memory,SingleVirtualDeviceMemoryLimit,%lld\n",
...
Status BaseGPUDeviceFactory::CreateDevices(
  ...
  // Gangmuk: 여기서의 VisibleDeviceCount()는 stream_executor/cuda/cuda_driver.cc에 있다.
  // CUDA_VISIBLE_DEVICES 환경변수로 걸러내고 남은 애들을 카운트 하는 거 같다. 
  // tf의 virtual device가 아니라.
  ...
  if (num_gpus_to_use > 0) {
    TF_RETURN_IF_ERROR(ParseVisibleDeviceList(gpu_options.visible_device_list(),
                                              &visible_gpu_order));
    printf("GPUDevice,visible_gpu_order.size(): %zu\n", visible_gpu_order.size());
    TF_RETURN_IF_ERROR(
        GetValidDeviceIds(visible_gpu_order, &valid_platform_gpu_ids));
    printf("GPUDevice,valid_platform_gpu_ids.size(): %zu\n", valid_platform_gpu_ids.size());
  }
  if (num_gpus_to_use > valid_platform_gpu_ids.size()) {
    num_gpus_to_use = valid_platform_gpu_ids.size();
  }
  printf("GPUDevice,num_gpus_to_use: %zu\n", num_gpus_to_use);
  ...
  // virtual_devices 생성하는 부분!
  const auto& virtual_devices = gpu_options.experimental().virtual_devices();
  // std::cout << "virtual_devices : " << typeid(virtual_devices).name() << std::endl;

  // old code
  // virtual_gpu = parser.parse_target_config("VirtualGPU");
  // num_virtual_gpu = parser.parse_target_config("NumVirtualGPU");
  // if (num_virtual_gpu >= 1 && vgpu_memory_limits.empty()) {
  //   for (int i = 0; i < num_virtual_gpu; i++) {
  //     // vgpu_memory_limits.push_back(parser.parse_target_config("GPUMemory"));
  //     vgpu_memory_limits.push_back(i);
  //   }
  // }
  // old code
  // if (virtual_gpu == 1) {
  //   if (virtual_gpu && vgpu_memory_limits.empty()) {
  //     vgpu_memory_limits.push_back(parser.parse_target_config("GPU_0_Memory"));
  //     vgpu_memory_limits.push_back(parser.parse_target_config("GPU_1_Memory"));
  //   }
  // }
  // printf("GPUDevice, vgpu_memory_limits.size(): %zu\n", vgpu_memory_limits.size());
  // if (!virtual_devices.empty()) {
  num_virtual_gpu = parser.parse_target_config("NumVirtualGPU");
  if (num_virtual_gpu > 0) {
    // TF_RETURN_IF_ERROR(VerifyVirtualDeviceSettings(num_gpus_to_use, gpu_options,
    //                                                visible_gpu_order,
    //                                                valid_platform_gpu_ids));
    // printf("before VerifyVirtualDeviceSettings\n");
    TF_RETURN_IF_ERROR(VerifyVirtualDeviceSettings(
        num_gpus_to_use, gpu_options, visible_gpu_order, valid_platform_gpu_ids,
        num_virtual_gpu));
    // printf("after VerifyVirtualDeviceSettings\n");
    // We've verified that num_gpus_to_use >= virtual_devices.size().
    // num_gpus_to_use = virtual_devices.size();

    // num_gpus_to_use = virtual_gpu;

    // printf("GPUDevice, num_gpus_to_use = virtual_devices.size()\n");
  ...
  // 여기서의 아래의 "num_gpus_to_use"는 platform_gpu를 말하는 거다
  // 그러니까 physically single gpu 머신에서는 아래 루프가 한번만 돌겠다.
  printf("GPUDevice,num_gpus_to_use: %zu\n", num_gpus_to_use);
  for (int i = 0; i < num_gpus_to_use; ++i) {
    ...
    // }
    // virtual device setting을 하면 else문으로 간다.
    // else {
    ...
    TF_RETURN_IF_ERROR(SingleVirtualDeviceMemoryLimit(
        gpu_options, platform_gpu_id, &single_virtual_device_memory_limit));
    if (num_virtual_gpu <= 0) {
      printf("GPUDevice,No VirtualGPU setup\n");
      memory_limit_bytes.push_back(single_virtual_device_memory_limit);
    }
    else {
      for (int i = 0; i < num_virtual_gpu; i++) {
        // int64 casted_memory_limit_bytes = vgpu_memory_limits[i];
        // casted_memory_limit_bytes = casted_memory_limit_bytes << 20;
        // memory_limit_bytes.push_back(casted_memory_limit_bytes);
        memory_limit_bytes.push_back(single_virtual_device_memory_limit);
        printf("GPUDevice,memory_limit_bytes[%d]: %lld\n", i,
               memory_limit_bytes[i] / (1024 * 1024));
      }
    }
    printf("GPUDevice,memory_limit_bytes.size(): %zu\n", memory_limit_bytes.size());
    // memory_limit_bytes.size() platform gpu(visible device) 가 가지고 있는
    // tf_gpu (virtual device) 갯수이다.
    while (next_tf_gpu_id < memory_limit_bytes.size()) {
      // while (next_tf_gpu_id < vgpu_memory_limits.size()) {
      TfGpuId tf_gpu_id(next_tf_gpu_id);
      ++next_tf_gpu_id;
      // 각 tf_gpu_id(virtual device)가 어떤 platform_gpu에 실제로 해당하는 지
      // 표시해준다.
      TF_RETURN_IF_ERROR(
          GpuIdManager::InsertTfPlatformGpuIdPair(tf_gpu_id, platform_gpu_id));
      // printf("TFGPUId[%d] - PlatformGPUId[%d] pair\n", next_tf_gpu_id,
      //        platform_gpu_id); // platform_gpu_id의 타입이 뭔지 몰라서 comment out
    }
  }
  const int num_tf_gpus = next_tf_gpu_id;
  printf("GPUDevice, num_tf_gpus: %d\n", num_tf_gpus);
  ...
  // CHECK_EQ(next_tf_gpu_id, vgpu_memory_limits.size());
  for (int di = 0; di < num_tf_gpus; ++di) {
    ...
    //////////////////////////////////////////////////////////////////////////////
    printf("GPUDevice,CreateGPUDevice, tf_gpu_id: %d\n", tf_gpu_id);
...
Status BaseGPUDeviceFactory::CreateGPUDevice(
  ...
  /* 
    Gangmuk
    GPUProcessState 객체 만들고(singleton()), 해당 gpu process state에
    gpu bfc allocator를 만든다.
  */
  ...
  /*
    Gangmuk
    해당 CreateGPUDevice 함수는 gpu_device_factory.cc에 있다.
    해당 함수는 GPUDevice class ctor를 부른다. GPUDevice class ctor는 
    BaseGPUDevice class ctor를 부른다. 여기에 max_streams=1 넣는다.
    BaseGPUDevice class는 다시 gpu_device.h에 있다.
  */
  // GPUDeviceFactory::CreateGPUDevice() in gpu_device_factory.cc
