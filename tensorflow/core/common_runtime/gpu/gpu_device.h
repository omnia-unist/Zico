...
class BaseGPUDevice : public LocalDevice {
 public:
  ...
  GM_Parser parser;
  // Gangmuk
  int recording_tensors;
  // bool skip_all_test;
  // bool skip_ref_count_is_one;
  int record_output_tensor_only;
  // bool tensor_recording_original_version;
  // bool throttle_kernel_launch;
  int max_launched_kernels;
  int forward_input_profiling_mode;
  int print_forward_input_or_allocate_output;
  int print_coordinator;
  std::string coordinator_policy;
  int print_raw_internal_csv;

  int inplace_profiling_step;
  int bfc_phase_two_step;
  ...
  int force_no_inplace;
  int print_forward_input;
  ...
  int ForceNoInplace() const override;
  ...
  // completes.
  // 이게 일종의 RecordingTensors()라고 보면 되겠다. 애초에 존재하기 때문에 새로 함수를 정의할 필요가 없다.
  // 위의 새로 정의한 함수들은 이 함수를 보고 따라 했다고 보면 되겠다.
 ...
 private:
  ...
  gtl::InlinedVector<StreamGroup*, 4> streams_; // Gangmuk: 왜 4지?
  mutex scratch_init_mutex_;
  gtl::InlinedVector<char*, 4> scratch_;
  // 여기에 스트림 담아 놓고 ComputeHelper 함수에서 보면 꺼내서 쓴다.
...
class BaseGPUDeviceFactory : public DeviceFactory {
 public:
  ...
  GM_Parser parser;
  int num_virtual_gpu;
  std::vector<int> vgpu_memory_limits;
