...
Node* Graph::AddNode(const NodeDef& node_def, Status* status) {
  ...
  // gangmuk: init
  node->num_bridge_edge = 0;
  node->having_bridge = false;
  if (node->name().find("gradients") != std::string::npos) {
    node->is_gradient = 1;
  } else {
    node->is_gradient = 0;
  }
...
const Edge* Graph::AddEdge(Node* source, int x, Node* dest, int y) {
  ...
  ++num_edges_;
	...
	// gangmuk
	// Marking the node if its name has "gradients" or "GradientDescent" or none of them.
	std::string src_node_name = source->name();
	std::string dst_node_name = dest->name();
	std::string str_gs = "gradients";
	std::string str_gd = "GradientDescent";
	std::size_t src_found_gs = src_node_name.find(str_gs);
	std::size_t src_found_gd = src_node_name.find(str_gd);
	std::size_t dst_found_gs = dst_node_name.find(str_gs);
	std::size_t dst_found_gd = dst_node_name.find(str_gd);

	// for source node
  // gradients
	if(src_found_gs != std::string::npos){
		source->node_category_ = 2;
		//std::cout<<"!! "<<str_gs<<", source id: "<<source->id()<<", name: "<<source->name()<<", category: "<<source->node_category_<<std::endl;
	}
  // GradientDescent
	else if(src_found_gd != std::string::npos){
		source->node_category_ = 3;
		//std::cout<<"!! "<<str_gd<<", source id: "<<source->id()<<", name: "<<source->name()<<", category: "<<source->node_category_<<std::endl;
	}
  // non-gradient
	else{
		source->node_category_ = 1;
		//std::cout<<"!! Non-Gradient, source id: "<<source->id()<<", name: "<<source->name()<<", category: "<<source->node_category_<<std::endl;
	}
  // potential error
	if(src_found_gs != std::string::npos && src_found_gd != std::string::npos){
		//std::cout<<"$$ WARNING: This node has \"gradients\" and \"GradientDescent\" !!!!"<<std::endl;
	}

	// for destination node
  // gradients
	if(dst_found_gs != std::string::npos){
		dest->node_category_ = 2;
		//std::cout<<"!! "<<str_gs<<", dest id: "<<dest->id()<<", name: "<<dest->name()<<", category: "<<dest->node_category_<<std::endl;
	}
  // GradientDescent
	else if(dst_found_gd != std::string::npos){
		dest->node_category_ = 3;
		//std::cout<<"!! "<<str_gd<<", dest id: "<<dest->id()<<", name: "<<dest->name()<<", category: "<<dest->node_category_<<std::endl;
	}
  // non-gradient
	else{
		dest->node_category_ = 1;
		//std::cout<<"!! Non-Gradient, dest id: "<<dest->id()<<", name: "<<dest->name()<<", category: "<<dest->node_category_<<std::endl;
	}
  // potential error
	if(dst_found_gs != std::string::npos && dst_found_gd != std::string::npos){
		//std::cout<<"$$ WARNING: This node has \"gradients\" and \"GradientDescent\" !!!!"<<std::endl;
	}

  // gangmuk
	int src_cate_ = source->node_category_;
	int dst_cate_ = dest->node_category_;
  int num_node_category = 3;
  e->edge_category_ = dst_cate_ + num_node_category*(src_cate_-1);
  e->is_bridge = false;

  /*
    This is the same logic as below one. Commented because it is duplicated.
  */
  // if (e->edge_category_ == 2) {
  //   e->is_bridge = true;
  // }

  // If edge connects from non-gradient to gradient node,
  // then, this edge is bridge edge.
  if (src_cate_ == 1 && dst_cate_ == 2) {
    e->is_bridge = true;
    source->having_bridge = true;
    source->num_bridge_edge++;
    source->bridge_output_idx.insert(x); //  the edge connects the xth output of `source` to the yth input of `dest`
    //printf("@@@@ source->num_bridge_edge: %d\n", source->num_bridge_edge);
  }
  //std::cout<<"!! from( "<<source->name()<<" : "<<src_cate_<<" )-> to( "<<dest->name()<<" : "<<dst_cate_<<"), edge_category: "<<e->edge_category_<<std::endl;
...
int Graph::InternDeviceName(const string& device_name) {
  ...
  // printf("Graph::InternDeviceName(%s) => index: %d. This device is newly added.\n", device_name.c_str(), index);
  return index;
}
