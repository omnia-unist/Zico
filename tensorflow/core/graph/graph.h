...
class Node {
 public:
  ...
  // Gangmuk
    ...
    // 3: GradientDescent
  int node_category_; 
  int node_category() { return node_category_; }
  int num_bridge_edge = 0;
  std::set<int> bridge_output_idx;
  bool having_bridge = false;
  int is_gradient = -2;
...
class Edge {
 public:
  ...
  // gangmuk
  // 1: non-gradient ===> non-gradient
  //*2: non-gradient ===> gradients ****
  // 3: non-gradient ===> GradientDescent

  // 4: gradients ===> non-gradient
  // 5: gradients ===> gradients
  // 6: gradients ===> GradientDescent

  // 7: GradientDescent ===> non-gradient
  // 8: GradientDescent ===> gradients
  // 9: GradientDescent ===> GradientDescent
  int edge_category_;
  int edge_category() { return edge_category_; }
  bool is_bridge; // true if src_node is non-gradient node and dst_node is gradients
...
class Graph {
 public:
  ...
  void CheckDeviceNameIndex(int index) {
    // printf("CheckDeviceNameIndex, index: %d, device_names_.size(): %d\n", 
    //         index, static_cast<int>(device_names_.size()));
    // fflush(stdout);
    DCHECK_GE(index, 0);
    DCHECK_LT(index, static_cast<int>(device_names_.size()));
    // printf("CheckDeviceNameIndex after, index: %d, device_names_.size(): %d\n", 
    //         index, static_cast<int>(device_names_.size()));
    // fflush(stdout);
  }
...
inline void Node::set_assigned_device_name_index(int index) {
  // printf("set_assigned_device_name_index before\n");
  // fflush(stdout);
  graph_->CheckDeviceNameIndex(index);
  // printf("set_assigned_device_name_index after\n");
  // fflush(stdout);
  
  // if (index ==2) {
  //   // if assgined device is GPU:0, force it to GPU:1.
  //   index = 3;
  // }
  // printf("set_assigned_device_name_index(%d), Node: %s\n", index, name().c_str());
