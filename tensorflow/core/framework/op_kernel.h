...
class OpKernelConstruction {
 public:
  bool print_1 = false; // print the most important cout
  bool print_2 = false; // print less important cout as well
...
class OpKernelContext {
 public:
  ...
  /*
    Gangmuk: static variable unordered map
    Record which nodes fail forward_input without recording tensor accesses.
  */
  static std::unordered_set<std::string> failed_RefCountIsOne_nodes_set;
  ...
  // Gangmuk
  // bool skip_all_test;
  // bool skip_ref_count_is_one;
  bool forward_input_profiling_mode;
  // bool print_forward_input_or_allocate_output;
  int inplace_profiling_step;
  bool force_no_inplace;
  bool print_forward_input;

  // Gangmuk
  /*
    record_output_tensor_only
    true: record output tensor access only
    false: record for all tensors access => original version
  */
  bool record_output_tensor_only;
  ...
  struct Params {
    ...
    // Gangmuk
    int is_gradient = -1;
    std::string from_which_node = "not_initialized";
    // int executorstate_id = -1;
    int executorimpl_id = -1;
  ...
  bool forward_input_to_output_with_shape(int input_index, int output_index,
                                          const TensorShape& output_shape,
                                          Tensor** output, int* reason_id) TF_MUST_USE_RESULT;
  ...
  std::unique_ptr<Tensor> forward_input(
      int input_index, int output_index, DataType output_dtype,
      const TensorShape& output_shape, MemoryType output_memory_type,
      const AllocatorAttributes& output_attr, int* reason_id) TF_MUST_USE_RESULT;
 ...
 private:
  ...
  // Gangmuk
  void record_tensor_reference(const Tensor& tensor, bool is_output);
  // Gangmuk
  void really_record_tensor_reference(const Tensor& tensor);

  // Internal common method used when allocating tensor memory
  // Gangmuk: newly added 3 Additional parameter, const char* caller
...
// Gangmuk
inline void OpKernelContext::record_tensor_reference(
    const Tensor& tensor, bool is_output) {
  DCHECK_EQ(params_->device->RequiresRecordingAccessedTensors(),
            params_->record_tensor_accesses);
  if (params_->record_tensor_accesses) {
    // !record_output_tensor_only => record all
    if (!record_output_tensor_only || is_output) {
      really_record_tensor_reference(tensor);
    }
  }
}
...
inline void OpKernelContext::NotifyUseOfPersistentTensor(const Tensor& t) {
  if (t.IsInitialized()) {
    ...
    // Gangmuk
    record_tensor_reference(t, true); // PersistentTensor must be recorded
  }
}
...
inline Status OpKernelContext::forward_input_or_allocate_output(
  ...
  int *reason_id = new int;
  int input_index_;
  bool forward_input_ret = false;
  for (int input_index : candidate_input_indices) {
    ...
    forward_input_ret = forward_input_to_output_with_shape(input_index, output_index,
                                           output_shape, output, reason_id);
    // Forwarding 성공했을 때,
    if (forward_input_ret) {
      delete reason_id;
      return Status::OK();
    }
  }
  ...
  // Gangmuk
  /*
    forward_input_profiling_mode: 처음에는 TensorRecording모드가 꺼져있고(원래
    텐서플로우가 돌아가는 방식), 안정적인 step_id 10 일 때(사실 이게 항상
    안정적인 단계라고 볼 수는 없다), forward_input_or_allocate_output에서
    forward_input를 시도했다가 실패하고 allocate_output을 결과적으로 하게 되는
    operator들이 있는데, 그 중에 RefCountIsOne인 이유로 실패한 친구들을
    기록한다. 이 친구들은 forward_input을 시도했다가 RefCount가 1이어서 실제
    진짜로 fail한 case이다. (true failed case)

    해당 true failed RefCountIsOne case를 기록하는 조건문
  */
  if (forward_input_profiling_mode && step_id() == inplace_profiling_step &&
      *reason_id == 7) {
    std::string node_name_with_index = op_kernel().name() + "-" +
                                       std::to_string(input_index_) + "-" +
                                       std::to_string(output_index);
    failed_RefCountIsOne_nodes_set.insert(node_name_with_index);
    if (print_forward_input) {
      printf(
          "**,forward_input_or_allocate_output,step_id,%d,%s,%s,"
          "InplaceProfiling\n",
  ...
  // if (true) {
  //   printf(
  //       "**,OpKernelContext::forward_input_or_allocate_output,Fail,%d,step_id,%d,%s,%s\n",
  //       *reason_id, step_id(), op_kernel().name().c_str(), op_kernel().type_string().c_str());
  // }
  delete reason_id;
...
inline Status OpKernelContext::forward_input_or_allocate_output(
  ...
  if (print_forward_input) {
    printf("**,forward_input_or_allocate_output using name,%s,%s,\n",
