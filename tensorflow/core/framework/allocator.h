...
class Allocator {
 public:
  ...
  static std::vector<void*> static_vector;
  ...
  // Gangmuk
  virtual int BFCId() {
    printf("GANGMUK_ERROR: This Allocator class BFCId() shouldn't have been called.\n");
    fflush(stdout);
    CHECK_EQ(0,1);
  }

  virtual int StepId() {
    printf("GANGMUK_ERROR: This Allocator class StepId() shouldn't have been called.\n");
    fflush(stdout);
    CHECK_EQ(0,1);
  }

  virtual void Set_StepId(int step_id) {
    printf("GANGMUK_ERROR: This Allocator class Set_StepId() shouldn't have been called.\n");
    fflush(stdout);
    CHECK_EQ(0,1);
  }

  virtual bool IsNewStep(int step_id) {
    printf("GANGMUK_ERROR: This Allocator class IsNewStep() shouldn't have been called.\n");
    fflush(stdout);
    CHECK_EQ(0,1);
  }

  virtual void UpdateNewStep(int step_id) {
    printf("GANGMUK_ERROR: This Allocator class UpdateNewStep() shouldn't have been called.\n");
    fflush(stdout);
    CHECK_EQ(0,1);
  }

  virtual void TagAsOutput(void* data) {
    // printf("[GANGMUK_ERROR]: This Allocator class TagAsOutput() shouldn't have been called.\n");
    printf("[GANGMUK] This is not BFCAllocator but %s. Do nothing\n", Name().c_str());
    fflush(stdout);
    // CHECK_EQ(0,1);
  }
  ...
  // from pure virtual function to normal virtual function
  virtual void* AllocateRaw(size_t alignment, size_t num_bytes, std::map< std::string, std::string > node_info){
    std::cout<<"!! WARNING 3 !!\n IT SHOULD HAVE BEEN OVERIDDEN\n";
  }
  ...
  // Gangmuk: Newly Added 3
  virtual void* AllocateRaw(size_t alignment, size_t num_bytes,
                            const AllocationAttributes& allocation_attr, std::map< std::string ,std::string > node_info) {
    // The default behavior is to use the implementation without any allocation
    // attributes.

    return AllocateRaw(alignment, num_bytes, node_info);
  }
  ...
  T* Allocate(size_t num_elements,
    ...
    void* p = AllocateRaw(kAllocatorAlignment, sizeof(T) * num_elements, allocation_attr, node_info);
    T* typed_p = reinterpret_cast<T*>(p);
    if (typed_p) RunCtor<T>(typed_p, num_elements);
    return typed_p;
  }
...
class SubAllocator {
 public:
  ...
  virtual void Free(void* ptr, size_t num_bytes) = 0;
	...
	// Gangmuk
	static std::mutex mtx_allocate;
	static std::mutex mtx_deallocate;
