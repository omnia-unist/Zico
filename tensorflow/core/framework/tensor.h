...
class Tensor {
 public:
  ...
  // Gangmuk: Newly added Tensor Constructor 3
  // This is for passing allocate_type to Tensor
  Tensor(Allocator* a, DataType type, const TensorShape& shape,
         const AllocationAttributes& allocation_attr,
 ...
 public:
 	...
 	// Gangmuk
	// Tensor Annotation whether it comes from BridgeNode or not.
	bool FromBridgeNode = false;
  ...
  // 원래는 private이었다. OpKernelContext::set_output에서 쓰기 위해서 public으로
  TensorBuffer* buf_;
...
//		std::cout<<"@@ ~TensorBuffer"<<std::endl;
	}
