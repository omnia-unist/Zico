...
class Buffer : public BufferBase {
 public:
  ...
  // Gangmuk: newly added 3 declaration. (Don't fornget declaration)
  Buffer(Allocator* a, int64 n, const AllocationAttributes& allocation_attr, std::map< std::string, std::string > node_info);
...
Buffer<T>::Buffer(Allocator* a, int64 n,
    ...
    : BufferBase(a, a->Allocate<T>(n, allocation_attr, node_info)), elem_(n) {}
...
//		std::cout<<"@@ ~Buffer() is calling alloc_->Deallocate()"<<std::endl;
...
void UnrefIfNonNull(core::RefCounted* buf) {
  if (buf){
		//std::cout<<"@@ UnrefIfNonNull() is calling buf->Unref()"<<std::endl;
		buf->Unref();
	}
}
...
void Tensor::CopyFromInternal(const Tensor& other, const TensorShape& shape) {
  ...
  if (buf_ != other.buf_) {
    ...
    buf_ = other.buf_;
    RefIfNonNull(buf_);
    // printf("**,CopyFromInternal, this.buf_ != nullptr\n");
  } else {
    // GangmuK: 실제로 출력됨.
    // printf("**,Tensor::CopyFromInternal, this.buf_ == nullptr\n");
  }
}
...
// Gangmuk: newly added Tensor Constructor 2 to pass alloc_type (persistent, temp, output)
//          Basically, it just prints out caller of this Tensor, OpKernel's name, OpKernel's type string
Tensor::Tensor(Allocator* a, DataType type, const TensorShape& shape,
               const AllocationAttributes& allocation_attr,
               std::map< std::string ,std::string > node_info)
    : shape_(shape), buf_(nullptr) {
  ...
  if (shape_.num_elements() > 0 || a->ShouldAllocateEmptyTensors()) {
//    CASES(type, buf_ = new Buffer<T>(a, shape.num_elements(), allocation_attr));
    CASES(type, buf_ = new Buffer<T>(a, shape.num_elements(), allocation_attr, node_info));
  }
  if (!allocation_attr.allocation_will_be_logged && buf_ != nullptr &&
      buf_->data() != nullptr && LogMemory::IsEnabled()) {
    LogMemory::RecordTensorAllocation("Unknown (with attributes)",
