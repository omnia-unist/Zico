...
// Gangmuk
//GlobalPool Allocator::allocator_global_pool_;
...
class CPUAllocator : public Allocator {
 public:
  ...
  void* AllocateRaw(size_t alignment, size_t num_bytes) override {
    // printf("CPUAllocator::AllocateRaw-1\n");
  ...
  // Gangmuk: Newly Added 3 but Not Changed at all from original functionality
  void* AllocateRaw(size_t alignment, size_t num_bytes, std::map< std::string ,std::string > node_info) override {
    // printf("CPUAllocator::AllocateRaw-3\n");
    if (num_bytes > LargeAllocationWarningBytes() &&
        single_allocation_warning_count_ < kMaxSingleAllocationWarnings) {
      ++single_allocation_warning_count_;
      LOG(WARNING) << "Allocation of " << num_bytes << " exceeds "
  ...
  void DeallocateRaw(void* ptr) override {
    // printf("CPUAllocator::DeallocateRaw\n");
    if (cpu_allocator_collect_stats) {
