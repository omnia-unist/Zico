...
// Gangmuk
ThreadPool::ThreadPool(Env* env, const string& name, int num_threads, bool low_latency_hint)
    : ThreadPool(env, ThreadOptions(), name, num_threads, low_latency_hint, nullptr) {}
...
ThreadPool::ThreadPool(Env* env, const ThreadOptions& thread_options,
  ...
  // Gangmuk: print하면 Error
  // std::cout << "ThreadPool Constructor, name: " << name << ", num_threads: " << num_threads << std::endl;
