...
/* static */ bool CUDADriver::CreateStream(CudaContext *context,
  ...
  // Gangmuk: 스트림을 기본적으로 0: default stream으로 만든다. 이게 concurrent
  // run을 막는 건가 싶다.
  ...
  // Gangmuk: tries
  // CUresult res = cuStreamCreate(out, 1);  // Gangmuk: 이렇게 해도 default stream으로 launching이 되네..
  // CUresult res = cuStreamCreate(out, 100);  // Gangmuk: 이렇게 하면 abort가 뜬다....
  // 0을 넣어도 default stream으로 안들어간다...
  // 결과적으로 0을 넣든 1을 넣든 똑같은 behavior를 보인다.
  CUresult res = cuStreamCreate(out, 0);

  std::cout << "@@ successfully created stream " << *out << " for context "
            << context << " on thread" << std::endl;
...
// Gangmuk: Overloading with mutex parameter
/* static */ void *CUDADriver::UnifiedMemoryAllocate(CudaContext *context,
                                                     uint64 bytes,
