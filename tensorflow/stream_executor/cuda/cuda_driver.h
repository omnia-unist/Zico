...
class CUDADriver {
 public:
    ...
    // Gangmuk: lock for two GPU Memory SubAllocator
...
//  static std::mutex mtx_deallocate;
  ...
  static void* UnifiedMemoryAllocate(CudaContext* context, uint64 bytes, std::mutex mtx_allocate_);
  ...
  static void UnifiedMemoryDeallocate(CudaContext* context, void* location, std::mutex mtx_deallocate_);
